...
const QuantityPopup = ({ title, id, item, onAddToCart, props, State, PhoneItem }) => {
    ...
    const handleAddToCart = async () => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:4000/cart/add`, {
                method: 'POST',
                headers: {
    ...
    const handleNavigate = async () => {
        ...
        try {
            console.log("Sending request to http://127.0.0.1:4000/phones/view/?id=" + id)
            const response = await fetch('http://127.0.0.1:4000/phones/view/?id=' + id)
            const data = await response.json();
            setPhoneData(data[0]);
    ...
    const getitemQuantity = async () => {
        if(loginctx.LoginStatus !== false && loginctx.LoginUserId !== ''){
            try {
                const response = await fetch("http://127.0.0.1:4000/cart/quantity", {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
